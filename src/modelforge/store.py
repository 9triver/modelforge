...
class ModelStore:
    ...
    def list_runs(self, model_id: str) -> list[dict]:
        ...
        for f in runs_dir.iterdir():
            if f.suffix == ".yaml":
                run = YAMLFile.read(f)
                # Exclude bulky fields from listing
                run.pop("log", None)
    ...
    def get_version_id_by_str(self, slug: str, version_str: str) -> str | None:
        """Get version UUID by version string (e.g. 'v1.0.0')."""
    ...
    def create_version_from_run(
        ...
        *,
        ...
        """Create a version record for a directory already populated by a pipeline run."""
        now = _now_str()
        # Normalize: strip leading "v" so display layer adds it consistently
        clean_version = version_str.lstrip("v") if version_str else version_str
        version_data = {
            "id": _new_id(),
            "version": clean_version,
            "description": f"Auto-generated by pipeline run",
    ...
    def next_version(self, slug: str, base_version: str) -> str:
        """Compute next version string, e.g. v1.0.0 -> v1.1.0."""
        versions_dir = self._model_dir(slug) / "versions"
    ...
    def create_draft_version(
        ...
        """Create a draft version by copying artifacts from base.

        Copies datasets/code/features/params but clears weights
        (weights are training output). Users can then modify
        artifacts before triggering training.
        """
        from fastapi import HTTPException
        ...
        if not base_vdir.exists():
            raise HTTPException(
                404, f"Base version '{base_version}' not found"
            )

        base_vyaml = base_vdir / "version.yaml"
        if not base_vyaml.exists():
            raise HTTPException(
                404, f"Base version '{base_version}' has no metadata"
            )
        ...
        if new_vdir.exists():
            raise HTTPException(
                409, f"Version '{next_ver}' already exists"
            )

        shutil.copytree(base_vdir, new_vdir)

        # Clear weights (training output, not input)
        weights_dir = new_vdir / "weights"
        if weights_dir.exists():
            shutil.rmtree(weights_dir)
        weights_dir.mkdir(exist_ok=True)

        # Remove old version.yaml
        ...
        version_data = {
            ...
            "description": (
                description
                or f"Draft based on {base_version}"
            ),
            "file_format": base_data.get(
                "file_format", "joblib"
            ),
            "file_path": None,
    ...
    def finalize_draft_version(
        ...
        *,
        ...
        """Finalize a draft after successful training.

        Transitions stage from draft to development, fills in
        weights path, file size, and metrics.
        """
        vyaml = vdir / "version.yaml"
        if not vyaml.exists():
            raise FileNotFoundError(
                f"version.yaml not found in {vdir}"
            )
        ...
        data["file_path"] = (
            weights_rel or f"weights/model.{file_format}"
        )
    ...
    def fork_model(
        ...
        """Fork: create a new model from a source model's version."""
        from fastapi import HTTPException
        ...
        # Create new model inheriting metadata
        ...
        # Write new version.yaml with lineage
        now = _now_str()
    ...
    def get_version_file_path(self, model_id: str, version_id: str) -> Path:
        ...
        if not data.get("file_path"):
            if data.get("stage") == "draft":
                raise HTTPException(
                    400,
                    "Cannot download from a draft version "
                    "(not yet trained)",
                )
            raise HTTPException(
                404, "No file associated with this version"
            )
    ...
    _MAX_ARTIFACT_SIZE = 50 * 1024 * 1024  # 50 MB

    _TEXT_EXTENSIONS = {
        ".py", ".yaml", ".yml", ".txt", ".json", ".md",
        ".cfg", ".ini", ".toml",
    }
        ...
        with open(dest, "wb") as f:
            while chunk := file.file.read(8192):
                ...
                if size > self._MAX_ARTIFACT_SIZE:
                    ...
                    raise HTTPException(
                        413,
                        f"File too large (max {self._MAX_ARTIFACT_SIZE // 1024 // 1024} MB)",
                    )
                f.write(chunk)
    ...
    def save_version_artifact_text(
        self,
        model_id: str,
        version_id: str,
        category: str,
